...
import { Plus, Database, Edit, Trash2, Eye, ToggleLeft, ToggleRight, Settings } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button icon={<Plus size={16} />} onClick={() => { reset(); setShowCreate(true); }}>
          Create Module
        </Button>
      </div>
            ...
            <Card key={mod.id} hover className="group">
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-surface-100 dark:border-surface-800">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => window.location.href = `/admin/modules/${mod.id}/fields`}
      ...
      <Modal
        ...
        onClose={() => { setShowCreate(false); setEditModule(null); reset(); }}
        title={editModule ? 'Edit Module' : 'Create New Module'}
        size="lg"
      >
        <form
          onSubmit={handleSubmit((data) => createMutation.mutate(data as unknown as Partial<Module>))}
          className="space-y-5"
        >
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="secondary" onClick={() => { setShowCreate(false); setEditModule(null); reset(); }}>
              Cancel
            </Button>
